...
const ImageLoader = (props) => {
    ...
    return (<div className={`w-full grow relative ${loaded ? 'bg-grey-800' : ''}`}>
        <img
            src={src}
            alt="img"
    ...
    return (
        <div className="w-[350px] h-[556px] mb-4 mx-4 overflow-hidden border-[5px] bg-component-light-bg dark:bg-component-dark-bg border-component-border rounded-[21px] flex flex-col">
            ...
            <div className="w-full h-fit px-8 py-7 font-body dark:text-white text-black">
                ...
                <h1 className="font-thin text-base leading-5 ml-3 my-5">Author: {author !== null ? author : "Anonymous"}<br />{publishedAt.toDateString()}<br />{company}</h1>
                <a className="text-lg my-auto font-title hover:underline" href={url}>Go To Page</a>
            </div>
        </div>
    )
}
